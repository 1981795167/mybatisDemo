...
<template:include ref="default.list">
	...
	<template:replace name="nav">
		<script>
			function addDoc(){
				Com_OpenWindow("${LUI_ContextPath}/chenyee/quotecenter/chenyee_qc_rom_ram/chenyeeQcRomRam.do?method=add");
			}
			function delDoc(){
				var values = [];
				...
				seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.post('${LUI_ContextPath}/chenyee/quotecenter/chenyee_qc_rom_ram/chenyeeQcRomRam.do?method=deleteall&categoryId=${param.categoryId}',$.param({"List_Selected":values},true),function(data){
								if(del_load!=null){
									del_load.hide();
		...
		<ui:combin ref="menu.nav.create">
			...
			<ui:varParam name="button">
				[
					<kmss:auth requestURL="/chenyee/quotecenter/chenyee_qc_rom_ram/chenyeeQcRomRam.do?method=add">
					{
	...
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			...
			<list:cri-auto modelName="com.landray.kmss.chenyee.quotecenter.model.ChenyeeQcRomRam" 
				property="fdRomRam;fdBomPrice;fdPurchasePrice;fdProductPrice;fdPresetPercent;fdPresetBottomPrice;fdProductId" />
		</list:criteria>
				...
				<tr>
					...
					<td align="right">
						<ui:toolbar count="3">
							...
							<kmss:auth requestURL="/chenyee/quotecenter/chenyee_qc_rom_ram/chenyeeQcRomRam.do?method=deleteall">					 								
							...
							<c:import url="/sys/introduce/import/sysIntroduceMain_cancelbtn.jsp" charEncoding="UTF-8">
								<c:param name="fdModelName" value="com.landray.kmss.chenyee.quotecenter.model.ChenyeeQcRomRam" />
							</c:import>
							--%>
							<%-- 修改权限 
							<c:import url="/sys/right/import/doc_right_change_button.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.chenyee.quotecenter.model.ChenyeeQcRomRam" />
							</c:import>
							--%>				 
						</ui:toolbar>
					</td>
		...
		<ui:fixed elem=".lui_list_operation"></ui:fixed>
	 	...
	 	<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/chenyee/quotecenter/chenyee_qc_rom_ram/chenyeeQcRomRam.do?method=data&categoryId=${param.categoryId}'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false"
				rowHref="/chenyee/quotecenter/chenyee_qc_rom_ram/chenyeeQcRomRam.do?method=view&fdId=!{fdId}"  name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial>
				 
				<list:col-auto props="fdRomRam;fdBomPrice;fdPurchasePrice;fdProductPrice;fdPresetPercent;fdPresetBottomPrice;fdProductId;docCreator.fdName"></list:col-auto>
			</list:colTable>
			<!-- 摘要视图 -->	
			<list:rowTable isDefault="false"
				rowHref="/chenyee/quotecenter/chenyee_qc_rom_ram/chenyeeQcRomRam.do?method=view&fdId=!{fdId}" name="rowtable" >
				...
				docSubject;icon;fdDescription;docCategory.fdName;docAuthor.fdName;docDept.fdName;docPublishTime;tag
				 --%>
			   	{"showOtherProps":"fdRomRam;fdBomPrice;fdPurchasePrice;fdProductPrice;fdPresetPercent;fdPresetBottomPrice;fdProductId;docCreator.fdName"}
				</list:row-template>
			</list:rowTable>
		</list:listview> 
