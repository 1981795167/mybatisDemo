	...
	<template:replace name="content">
		<c:choose>
			<c:when test="${param.filter == '1' }">
				<c:import url="/chenyee/quotecenter/chenyee_qc_apply/mobile/filter.jsp" charEncoding="UTF-8">
				</c:import>
			</c:when>
			<c:otherwise>
				<c:import url="/chenyee/quotecenter/chenyee_qc_apply/mobile/listview.jsp" charEncoding="UTF-8">
				</c:import>
			</c:otherwise>
		</c:choose>
