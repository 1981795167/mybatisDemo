...
<template:include ref="default.view">
	...
	<template:replace name="toolbar">
		...
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
			<kmss:auth requestURL="/chenyee/quotecenter/chenyee_qc_purchase/chenyeeQcPurchase.do?method=edit&fdId=${param.fdId}" requestMethod="GET">
				<ui:button text="${lfn:message('button.edit')}" 
							onclick="Com_OpenWindow('chenyeeQcPurchase.do?method=edit&fdId=${param.fdId}','_self');" order="2">
				</ui:button>
			</kmss:auth>
			<kmss:auth requestURL="/chenyee/quotecenter/chenyee_qc_purchase/chenyeeQcPurchase.do?method=delete&fdId=${param.fdId}" requestMethod="GET">
				<ui:button text="${lfn:message('button.delete')}" order="4"
							onclick="deleteDoc('chenyeeQcPurchase.do?method=delete&fdId=${param.fdId}');">
				</ui:button> 
			</kmss:auth>
	...
	<template:replace name="content">
		<div class='lui_form_title_frame'>
			...
			<div class='lui_form_baseinfo'>
				<%--
				${ lfn:message('chenyee-quotecenter:chenyeeQcPurchase.docCreator') }：
				<ui:person bean="${chenyeeQcPurchase.docCreator}"></ui:person>&nbsp;
				<c:if test="${ not empty chenyeeQcPurchaseForm.docPublishTime }">
					<bean:write name="chenyeeQcPurchaseForm" property="docPublishTime" />
				</c:if>&nbsp;
				<c:if test="${chenyeeQcPurchaseForm.docStatus == '30'}">
				 <bean:message key="sysEvaluationMain.tab.evaluation.label" bundle="sys-evaluation"/>
					 <span data-lui-mark='sys.evaluation.fdEvaluateCount' class="com_number">
						 <c:choose>
						   <c:when test="${not empty chenyeeQcPurchaseForm.evaluationForm.fdEvaluateCount}">
						      ${ chenyeeQcPurchaseForm.evaluationForm.fdEvaluateCount }
						   </c:when>
						   <c:otherwise>(0)</c:otherwise>
						 </c:choose>
					 </span>
				</c:if>
				<bean:message key="sysReadLog.tab.readlog.label" bundle="sys-readlog"/><span data-lui-mark="sys.readlog.fdReadCount" class="com_number">(${ chenyeeQcPurchaseForm.readLogForm.fdReadCount })</span>
				 --%>
			</div>
		</div>
		<%-- 文档概览
		<c:if test="${ not empty chenyeeQcPurchaseForm.fdDescription }">
			<div class="lui_form_summary_frame">			
				<bean:write	name="chenyeeQcPurchaseForm" property="fdDescription" />
			</div>
		</c:if>
		--%>
		<div class="lui_form_content_frame">
			<%-- 文档内容 --%>
			<c:if test="${not empty chenyeeQcPurchaseForm.docContent}">
				<div style="min-height: 200px;">
					${chenyeeQcPurchaseForm.docContent }	
				</div>			
			</c:if>
			<%-- 其它字段 --%>
			<table class="tb_simple" width=100%>
				...
				<tr>
					...
					<td width="35%">
						<c:out value="${chenyeeQcPurchaseForm.docCreatorName}" />
					</td>
		...
		<ui:accordionpanel style="min-width:200px;"> 
			<ui:content title="${ lfn:message('sys-doc:kmDoc.kmDocKnowledge.docInfo') }" toggle="false">
				<c:import url="/sys/evaluation/import/sysEvaluationMain_view_star.jsp" charEncoding="UTF-8">
					<c:param name="formName" value="chenyeeQcPurchaseForm" />
				</c:import>
				<ul class='lui_form_info'>
					<li><bean:message bundle="chenyee-quotecenter" key="chenyeeQcPurchase.docCreator" />：
					<ui:person personId="${chenyeeQcPurchaseForm.docCreatorId}" personName="${chenyeeQcPurchaseForm.docCreatorName}"></ui:person></li>
					<li><bean:message bundle="chenyee-quotecenter" key="chenyeeQcPurchase.docDept" />：${chenyeeQcPurchaseForm.docDeptName}</li>
					<li><bean:message bundle="chenyee-quotecenter" key="chenyeeQcPurchase.docStatus" />：<sunbor:enumsShow value="${chenyeeQcPurchaseForm.docStatus}" enumsType="common_status" /></li>
					<li><bean:message bundle="chenyee-quotecenter" key="chenyeeQcPurchase.docCreateTime" />：${chenyeeQcPurchaseForm.docCreateTime }</li>				
				</ul>
			</ui:content>
		</ui:accordionpanel>
	</template:replace>
